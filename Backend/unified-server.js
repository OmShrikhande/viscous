...
const { 
  ...
  resetStopsReached,
  checkMidnightReset
} = require('./Tracking/services/locationService.js');

// Import enhanced location service
const { enhancedLocationService } = require('./Tracking/services/enhancedLocationService.js');

// Import optimized Excel tracking service (replaces bidirectional service)
const { optimizedExcelTrackingService } = require('./Tracking/services/optimizedExcelTrackingService.js');
...
const userRoutes = require('./Bus tracking Admin/Admin/Backend/routes/userRoutes.js');
...
/**
 * Rotates serial numbers so that the reached stop becomes 1,
 * the next stop becomes 2, and so on, wrapping around for all stops.
 * POST /tracking/api/rotate-serials/:stopId
 * Body: { }
 *
 * Example:
 * Original order by serial: [StopA:1, StopB:2, StopC:3, StopD:4, StopE:5]
 * If stopId = StopC → New order:
 * StopC → 1, StopD → 2, StopE → 3, StopA → 4, StopB → 5
 */
app.post('/tracking/api/rotate-serials/:stopId', async (req, res) => {
  try {
    ...
    // Sort by current serialNumber ascending (true route order)
    const sorted = stops.sort((a, b) => a.serialNumber - b.serialNumber);

    // Find index of reached stop
    const startIndex = sorted.findIndex(s => s.id === stopId);
    if (startIndex === -1) {
      return res.status(404).json({ success: false, message: 'Stop not found' });
    }

    // Rotate array so reached stop is first
    const rotated = [
      ...sorted.slice(startIndex),
      ...sorted.slice(0, startIndex)
    ];

    // Assign new serials: reached stop gets 1, next gets 2, etc.
    const updates = rotated.map((stop, index) => ({
      id: stop.id,
      newSerial: index + 1
    }));
    ...
    res.json({
      success: true,
      message: 'Serial numbers rotated successfully',
      updates
    });
  } catch (error) {
