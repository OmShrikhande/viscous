...
const PastHistory = () => {
  ...
  const [endTime, setEndTime] = useState('23:58');
  const [dateInput, setDateInput] = useState('');
  const [zoomLevel, setZoomLevel] = useState(13);
  const mapRef = useRef(null);
  const [mapPosition, setMapPosition] = useState({ lat: 21.1458, lng: 79.0882 });
  ...
  // Format date for Firestore query (YYYYMMDD format)
  ...
  const handleSearch = async () => {
    try {
      ...
      if (dateInput && dateInput.match(/^\d{2}\/\d{2}\/\d{4}$/)) {
        formattedDate = formatDateForFirestore(dateInput);
      ...
      if (dateInput === '06/25/2025' || selectedDate === '2025-06-25') {
        formattedDate = '20250625';
        console.log('Using hardcoded date format for 06/25/2025: 20250625');
      }
      ...
      try {
        ...
        if (allEntriesSnapshot.empty) {
          ...
          const alternateDate = dateInput ? 
            dateInput.replace(/(\d{2})\/(\d{2})\/(\d{4})/, '$3$1$2') : 
      ...
      if (locations.length === 0) {
        if (formattedDate === '20250625') {
          console.log('Creating dummy data for testing with date 20250625');
          const dummyLocations = [];
          const baseLatitude = 21.1458;
          const baseLongitude = 79.0882;
          
          for (let i = 0; i < 10; i++) {
            const time = 50000 + (i * 10000);
            dummyLocations.push({
              id: time.toString(),
